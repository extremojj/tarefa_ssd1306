...
// Função para enviar um comando para o display SSD1306
...
// Função para enviar os dados para o display SSD1306
...
void ssd1306_pixel(ssd1306_t *ssd, uint8_t x, uint8_t y, bool value) {
  uint16_t index = x + (y / 8) * ssd->width;  // Correção no cálculo do índice
...
// Função para preencher todo o display SSD1306
...
// Função para desenhar um retângulo no display SSD1306
...
// Função para desenhar uma linha no display SSD1306
...
void ssd1306_draw_char(ssd1306_t *ssd, char c, uint8_t x, uint8_t y) {
  uint16_t index = 0;
  if (c >= 'A' && c <= 'Z') {
    index = (c - 'A') * 8;  // Ajuste no índice para caracteres maiúsculos
  } else if (c >= '0' && c <= '9') {
    index = (c - '0') * 8;  // Ajuste no índice para números
  }
...
// Função para desenhar uma string no display SSD1306
